...
public class AuthController {
    ...
    @GetMapping("/get/name")
    public String getusername(@RequestParam(value="name", defaultValue = "이강현") String username,
                              @RequestParam(required = false) Integer age) {
        System.out.println(username + age);
        return username + age;
    }
    //안에서 사용하는 변수명과 쿼리스트링의 키값이 다를 경우 괄호안에 표기해주면 됨
    //그리고 기본값도 설정이 가능함
    //그리고 다른 타입도 가능하며 여러개의 RequestParam도 받을 수 있다
    //int는 null을 허용안하기 때문에 값이 없음의 상태
    //그래서 required = false를 했지만 에러가 뜸 => Integer로 해야 null을 받을 수 있다
